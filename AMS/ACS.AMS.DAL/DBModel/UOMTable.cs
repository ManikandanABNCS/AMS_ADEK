...
public partial class UOMTable : BaseEntityObject, IACSDBObject
{
    [Key]
    ...
    [InverseProperty("UOMTableCreatedByNavigation")]
    ...
    [InverseProperty("UOMTableLastModifiedByNavigation")]
...
/*
	...
	/// <summary>
    /// Default constructor for UOMTable
    /// </summary>
    /// <param name="db"></param>
	public UOMTable(AMSContext _db)
	{
...
*/
    ...
    {
		if(!string.IsNullOrEmpty(this.UOMCode)) this.UOMCode = this.UOMCode.Trim();
		...
		if(!string.IsNullOrEmpty(this.UOMName)) this.UOMName = this.UOMName.Trim();
    ...
    public static UOMTable GetItem(AMSContext _db, int id)
    {
        return (from b in _db.UOMTable
                where b.UOMID == id
                select b).FirstOrDefault();
    }

    public static IQueryable<UOMTable> GetAllItems(AMSContext _db , bool includeInactiveItems = false )
    {
        if(includeInactiveItems)
        {
            return (from b in _db.UOMTable
                    where b.StatusID != (int) StatusValue.Deleted 
                    orderby b.UOMID descending
                    select b);
        }
        else
        {
    ...
    public static IQueryable<UOMTable> GetAllUserItems(AMSContext _db, int userID, bool includeInactiveItems = false)
    {
        return UOMTable.GetAllItems(_db, includeInactiveItems);
    }
    ...
    {
        return (from b in _db.UOMTable select b);
    }
    ...
    {
        return UOMTable.GetAllUserItems(_db, userID, includeInactiveItems);
    }

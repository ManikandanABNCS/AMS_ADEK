...
namespace ACS.AMS.DAL
{
    public static class DBHelper
    {
                ...
                using (SqlCommand command = cn.CreateCommand())
                {
                    ...
                    else
                    {
                        ...
                        //Pass null for all its parameters
                        {
                            ...
                            {
                                command.Parameters.AddWithValue(r["PARAMETER_NAME"] + "", GetDefaultParamValue(r["PARAMETER_NAME"] + ""));
                    ...
                    if(tbl == null)
                    {
                        ...
                        //generate schema by executing query
                        ...
                        {
                            var recCOunt = ds3.Tables[0].Rows.Count;

                            var col1 = ds3.Tables[0].Columns[10];
                            var col2 = ds3.Tables[0].Columns[10];
                            var col23 = ds3.Tables[0].Columns[0];
                        }
                    }
        ...
        private static object GetDefaultParamValue(string paramName)
        {
            switch(paramName.ToUpper())
            {
                case "@LANGUAGEID":
                    return SessionUserHelper.LanguageID;

                case "@USERID":
                    return SessionUserHelper.UserID;

                case "@COMPANYID":
                    return SessionUserHelper.CompanyID;
            }

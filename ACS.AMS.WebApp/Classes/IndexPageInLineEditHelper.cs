...
{
    public class IndexPageInLineEditHelper<T>
    {
        public IndexPageInLineEditHelper(IndexPageModel model) 
        {
            Model = model;
        ...
        public void CreateGridControl(IHtmlHelper<T> htmlHelper, bool IsLineItem=false)
        {
            ...
            AddGridActionColumns(gridCtrl, Url, gridColumnIndexName, _rightName, IsLineItem);

            //Configure Grid data source
            ConfigureGrid(gridCtrl, IsLineItem, readDataHandler: Model.ReadDataHandler, readRouteValues: Model.ReadRouteValues, readActionName: Model.ReadActionName);
        ...
        public void ConfigureGrid(GridBuilder<dynamic> grid,bool IsLineItem=false, int pageSize = 10, bool pageSizesRequired = true,
                               ...
                               object readRouteValues = null, string readActionName = "_Index")
        {
            ...
            string deleteActionName = IsLineItem == true ? "_DeleteLineItem" : "_Delete";
            ...
            grid.Editable(editable => editable.Mode(GridEditMode.InLine));
            ...
            grid.DataSource(dataSource => dataSource
                    ...
                    .Read(read => read.Action(readActionName, controllerName, readRouteValues).Data(readDataHandler))
                        .Destroy(destroy => destroy.Action(deleteActionName, controllerName))
                );
        }

        public void AddGridActionColumns<T>(GridBuilder<T> grid, IUrlHelper url, string indexName, string rightName,bool IsLineItem = false,
         ...
         string detailsAdditionalData = null, string editAdditionalData = null, string deleteAdditionalData = null, string editableFieldName = "") where T : class
        {
            ...
            AddGridActionColumns(grid, url, result, rightName, IsLineItem, editScriptFunctionName, deleteScriptFunctionName, gridName, detailsActionName, editActionName, 
                deleteActionName, detailsAdditionalData, editAdditionalData, deleteAdditionalData, editableFieldName);
        }

        public void AddGridActionColumns<T>(GridBuilder<T> grid, IUrlHelper url, List<UserGridNewColumnTable> columnList, string rightName, bool IsLineItem=false,
         string editScriptFunctionName = "editRecord", string deleteScriptFunctionName = "deleteRecord",
         string gridName = "DetailsGrid",
         string detailsActionName = "Details", string editActionName = "Edit", string deleteActionName = "Delete",
         ...
         string editableFieldName = "") where T : class
        {
            var controllerName = this.ControllerName;
            var primaryKeyName = PrimaryKeyFieldName;
            deleteActionName = IsLineItem == true ? "_DeleteLineItem" : deleteActionName;
            ...
            if (IsLineItem==true)
            deleteAdditionalData = (deleteAdditionalData + "&" ?? string.Empty) + "currentPageID=" + Model.EntityInstance.CurrentPageID;
            grid.Columns(col =>
            {
                ...
                foreach (var d in result)
                {
                    ...
                    {
                        //string[] referenceTables = d.MasterGridLineItemTable.FieldName.ToString().Split('.');
                    ...
                    .Model(model =>
                    {
                        model.Field(d.MasterGridLineItem.FieldName, type).Editable(true);
                    })
                    .Update(update => update.Action("Edit", controllerName, new { primaryKeyID = d.MasterGridLineItem.MasterGridLineItemID, pageName = PageName, currentPageID = Model.EntityInstance.CurrentPageID }))
                    );
                ...
                col.Command(command => { command.Edit();}).Width(200);
